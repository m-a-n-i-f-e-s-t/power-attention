...
@torch.library.custom_op("power::fused_proj_xent_fwd_bwd", mutates_args=())
def fused_proj_xent_fwd_bwd(in_feat: torch.Tensor, proj_weight: torch.Tensor, targ: torch.Tensor, n_loop_iters: int, ignore_index: int, reduction: str) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    return loss, grad_in_feat, grad_proj_weight


@fused_proj_xent_fwd_bwd.register_fake
def fused_proj_xent_fwd_bwd_fake(in_feat: torch.Tensor, proj_weight: torch.Tensor, targ: torch.Tensor, n_loop_iters: int, ignore_index: int, reduction: str):
    return (torch.empty([], device=in_feat.device, dtype=in_feat.dtype), 
            torch.empty(in_feat.shape, device=in_feat.device, dtype=in_feat.dtype),
            torch.empty(proj_weight.shape, device=proj_weight.device, dtype=proj_weight.dtype))
...
class FusedCrossEntropyLossFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        loss, grad_in_feat, grad_proj_weight = fused_proj_xent_fwd_bwd(in_feat, proj_weight, targ, n_loop_iters, ignore_index, reduction)
...
if __name__ == '__main__':    
    ...
    # Initialize model
    model = FusedProjectionPlusCrossEntropyLoss(n_embd, vocab_size)
    model.to('cuda')

    # Create random input features and targets
    x = torch.randn(batch_size * seq_len, n_embd, requires_grad=True, device='cuda')
    targets = torch.randint(0, vocab_size, (batch_size * seq_len,), device='cuda')
    
    # Forward pass
    loss = model(x, targets, n_loop_iters=4)
    ...
    # Compare against naive PyTorch implementation
    x = torch.tensor(x.clone().detach(), requires_grad=True)
    weight = torch.tensor(model.weight.clone().detach(), requires_grad=True)
    
    # Naive implementation using standard PyTorch ops
    logits = x @ weight.T
    naive_loss = torch.nn.functional.cross_entropy(
        logits, 
        targets,
        ignore_index=-100,
        reduction='mean'
    ...
    # Compare losses
    print(f"Fused loss: {loss.item():.6f}")
    print(f"Naive loss: {naive_loss.item():.6f}")
    print(f"Loss diff: {abs(loss - naive_loss).item():.6f}")
    
    # Compare gradients
    grad_diff_x = (x.grad - x_grad).abs().max()
    grad_diff_w = (weight.grad - weight_grad).abs().max()
    print(f"Max gradient diff (x): {grad_diff_x:.6f}")
    print(f"Max gradient diff (weight): {grad_diff_w:.6f}")

    # Test with torch.compile
    print("\nTesting with torch.compile...")
    
    # Reset model and inputs
    model = FusedProjectionPlusCrossEntropyLoss(n_embd, vocab_size)
    model.to('cuda')
    ...
    # Forward and backward with compiled model
    loss = model(x, targets, n_loop_iters=4)
    loss.backward()
    ...
    assert x.grad is not None
    assert model.weight.grad is not None
    print("Compiled model forward/backward pass successful")